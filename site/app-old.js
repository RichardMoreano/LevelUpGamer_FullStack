...
// Variables globales para acceder a los datos
// Las variables productosBase, regiones, categorias se cargan desde datos.js
// Las funciones de storage se definen aquí
...
/* =============== Primera carga =============== */
// Esperar a que se carguen los datos globales
...
// Llamar después de que se cargue la página
...
function sanearLocalStorage(){
  ...
  if (!Array.isArray(prods)) {
    ...
    prods = Array.isArray(window.productosBase) ? window.productosBase : [];
...
function inicializarRegistro(){
  ...
  if (selRegion && selComuna) {
    if (typeof window.regiones === "undefined" || !Array.isArray(window.regiones)) {
      console.warn("No se encontró 'regiones'. Asegúrate de cargar datos.js antes de app.js.");
    } else {
      selRegion.innerHTML = window.regiones.map(r => `<option>${r.nombre}</option>`).join("");
      const actualizarComunas = () => {
        const r = window.regiones.find(x => x.nombre === selRegion.value);
  ...
  form.addEventListener("submit", async (e)=>{
    ...
    const nuevo = { 
      ...
      comuna, 
      direccion,
      password: pass, // La API espera 'password', no 'pass'
      descuentoDuoc, 
      puntosLevelUp:0, 
      codigoReferido
    };
    ...
    const resultadoAPI = await registrarUsuarioAPI(nuevo);
    ...
    if (resultadoAPI.exito) {
      ...
      // Si falla la API, usar almacenamiento local como respaldo
      console.warn('Registro en API falló, usando almacenamiento local');
      const nuevoLocal = { ...nuevo, pass: nuevo.password, compras: [] };
      delete nuevoLocal.password;
      usuarios.push(nuevoLocal);
      guardar("usuarios", usuarios);
      
      const msg = document.getElementById("msgRegistro");
      if (msg) msg.textContent = "¡Cuenta creada localmente! Ahora podés ingresar.";
      form.reset();
    }
  });
}
...
function inicializarLogin(){
  ...
  form.addEventListener("submit", async (e)=>{
    ...
    // Intentar login con la API primero
    if (msg) msg.textContent = "Iniciando sesión...";
    const resultadoAPI = await loginUsuarioAPI(correo, pass);
    
    if (resultadoAPI.exito) {
      if (msg) msg.textContent = "¡Bienvenido/a, " + (resultadoAPI.usuario.nombres||"") + "! Redirigiendo…";
      setTimeout(()=>{ window.location.href = "productos.html"; }, 700);
    } else {
      // Si falla la API, intentar con datos locales
      console.warn('Login con API falló, intentando con datos locales');
      const usuarios = obtener("usuarios", []);
      const u = usuarios.find(x=>x.correo.toLowerCase()===correo.toLowerCase());
      
      if(!u){ 
        if (errCorreo) errCorreo.textContent = "No existe el usuario."; 
        if (msg) msg.textContent = "";
        return; 
      }
      if(u.pass !== pass){ 
        if (errPass) errPass.textContent = "Contraseña incorrecta."; 
        if (msg) msg.textContent = "";
        return; 
      }

      // Login local exitoso
      guardar("sesionActual", u);
      guardar("sesion", {correo:u.correo, tipo:u.tipoUsuario});
      if (msg) msg.textContent = "¡Bienvenido/a, " + (u.nombres||"") + "! (Local) Redirigiendo…";
      setTimeout(()=>{ window.location.href = "productos.html"; }, 700);
    }
  });
}
...
async function registrarUsuarioAPI(usuario) {
  try {
    ...
    if (response.ok) {
      ...
      console.log('Usuario registrado en API:', resultado);
      return { exito: true, usuario: resultado };
    } else {
      const error = await response.text();
      console.warn('Error en registro API:', error);
      return { exito: false, mensaje: error };
    }
  } catch (error) {
    console.warn('Error de conexión al registrar:', error);
...
async function loginUsuarioAPI(correo, password) {
  try {
    ...
    if (response.ok) {
      ...
      guardar("sesionActual", resultado.usuario);
      guardar("token", resultado.token);
    ...
    console.warn('Error de conexión al hacer login:', error);
    return { exito: false, mensaje: 'Error de conexión' };
  }
}
...
async function renderDestacados(){
  ...
  const prods = await cargarProductosDesdeAPI();
  const productosDestacados = prods.slice(0,6);
  cont.innerHTML = productosDestacados.map(p=>`
...
async function popularCategorias(){
  ...
  // Usar categorías predefinidas o extraer de productos
  let cats = [];
  if (window.categorias && Array.isArray(window.categorias)) {
    cats = window.categorias;
  } else {
    const prods = await cargarProductosDesdeAPI();
    cats = Array.from(new Set(prods.map(p=>p.categoria))).sort();
  }
...
async function renderProductos(){
  ...
  // Cargar productos desde API
  const todosProductos = await cargarProductosDesdeAPI();
  const prods = todosProductos.filter(p=>{
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Index
  if (document.getElementById("gridDestacados")) {
    renderDestacados();
  }
